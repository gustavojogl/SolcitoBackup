...
class common extends StormComponent
{
	...
	public function checkInstall()
	{
		...
		if (count($backups) == 0)
		{
			$errors[] = array('type' => 'warning', 'title' => 'Informacion Cron job', 'message' => 'Cada 5 minutos: '."\n\n".'php '.  realpath(rtrim(Storm::ToAbsolute('../cron.php'))) . ' > /dev/null 2>&1');
		}
		...
		foreach ($backups as $backup)
		{
			if (isset($backup->errors))
			{
				...
				{
					$errors[] = array('type' => 'error', 'title' => '`'. $backup->title .'` failed', 'message' => 'The error is: '. $error->message);
				}
			...
			if (isset($backup->warnings))
			{
				...
				{
					$errors[] = array('type' => 'warning', 'title' => '`'. $backup->title .'` had warnings', 'message' => $error->message);
				}
