...
class backup_model extends StormModel
{
	...
	private function walk_and_backup($dir, $zip, $cd, $zipCd, $excluded, $ignores, $isRoot = false)
	{
		...
		foreach ($dir->GetFiles($cd) as $file)
		{
			if (!$this->isIgnored($file, $ignores))
			{
				...
				{
					$this->warnings['nonReadableFiles']['message'] = "Some files cannot be read (because of OS permissions) and were not included in the archive.";
	...
	public function quickBackup($data)
	{
		...
		try
		{
			$zip = new ZipDirectory($tempFilePath, ZipArchive::CREATE);
			...
			if ($importSql && $backup->hasDatabase)
			{
				...
				if ($sqlDirStats !== false)
				{
					...
					for ($i = 0; $i < $zip->numFiles; $i++)
					{
						...
						$name = substr($sqlFileName, 0, strpos($sqlFileName, '.'));
						$ext = substr($sqlFileName, strpos($sqlFileName, '.') + 1);
