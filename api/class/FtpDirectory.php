...
class FtpDirectory implements IDirectory
{
	...
	public function GetLocalPath($path, $relative = false)
	{
		if ($relative)
			return rtrim($path, '/');
		else
	...
	{
		return ftp_size($this->ftp, $this->GetLocalPath($path, true));
	}
	...
	{
		return !preg_match('/\./', $this->GetLocalPath($path, true));
		//return (@ftp_size($this->ftp, $path) == -1);
	}

	public function GetDirs($dir = '')
	{
		$parent = $this->GetLocalPath($dir, true);
	...
	public function IsReadable($path)
	{
		...
		{
			// Directory
			$res = @ftp_chdir($this->ftp, $this->GetLocalPath($path, true));

			ftp_chdir($this->ftp, $this->cd);
	...
	public function IsWritable($path)
	{
		...
		{
			// Directory
			if (@ftp_put($this->ftp, $this->GetLocalPath('.is_writable_test_file', true), Storm::ToAbsolute('do-not-remove'), FTP_ASCII))
			{
				@ftp_delete($this->ftp, $this->GetLocalPath('.is_writable_test_file', true));
	...
	{
		$res = ftp_put($this->ftp, $this->GetLocalPath($to, true), $from, FTP_BINARY);
	...
	{
		$res = ftp_delete($this->ftp, $this->GetLocalPath($path, true));
	...
	{
		$uploaded = ftp_mkdir($this->ftp, $this->GetLocalPath($path, true));
		$chmoded  = ftp_chmod($this->ftp, $mode, $this->GetLocalPath($path, true));
